...
class DBManager:
    def __init__(self, db_path=None):
        # Sử dụng URL và Key từ Secrets
        self.url = st.secrets["SUPABASE_URL"]
        self.key = st.secrets["SUPABASE_KEY"]
        self.supabase: Client = create_client(self.url, self.key)
        
        # Giả lập các thuộc tính cũ để không làm gãy code bên ngoài
        self.db_path = db_path
        
        # Lưu ý: Supabase tạo bảng qua giao diện SQL Editor. 
        # Các hàm create_tables và upgrade_tables được giữ lại logic để kiểm tra cấu trúc.
        self.create_tables()
        self.upgrade_tables() 
